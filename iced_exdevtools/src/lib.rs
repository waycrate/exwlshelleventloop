    ...
    P: Program + 'static,
{
    ...
    pub fn subscription(&self, program: &P) -> Subscription<Event<P>> {
        ...
        let hotkeys = futures::keyboard::on_key_press(|key, _modifiers| match key {
            keyboard::Key::Named(keyboard::key::Named::F12) => Some(Message::ToggleComet),
            _ => None,
        })
        .map(Event::Message);
    ...
    column![
        ...
        container(
            scrollable(
                column(
                    logs.iter()
                        .map(|log| { text(log).size(12).font(Renderer::MONOSPACE_FONT).into() }),
                )
                .spacing(3),
            )
            .spacing(10)
